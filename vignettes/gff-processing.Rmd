...
--!>
```{r gff-workflow-plot, eval=FALSE}
plot(result)
```
![plot of chunk gff-workflow-plot](gff-workflow-plot-1.png)
...
Here is a list of all currently supported getter functions.

 * `get_parents`  - list of node parents
 * `get_value`    - the value stored in the object, if any
 * `get_ok`       - is the node in a passing state?
 * `get_code`     - the code chunk (minus the docstring)
 * `get_error`    - any errors that were raised
 * `get_warnings` - any warnings that were raised
 * `get_notes`    - any messages that were printed
 * `get_doc`      - the docstring
 * `get_time`     - the time required to evaluate the chunk
 * `get_mem`      - the size of the resulting value, in bytes
 * `get_branch`   - nodes branching from this node (not used in this pipeline)
