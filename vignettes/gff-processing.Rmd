...
# Below are a few variants that occur (unfortunately) in the wild
...
For mRNA and exon, I am merging ontology terms (mRNA and transcript; exon
...
* Validate GFF files of various formats
* Generate informative error messages for GFF intervals that are hierarchical
* Merge synonymous ontology terms
...
read_gff(good_gfffile) # which file is this
```
...
Notice how row 4 has a NA for the start position. After examining the original file, we can see that the numeric start position is a string "one thousand eight hundred and seven". Not a common mistake. A more common mistake would be adding commas such as 1,807 which R would load as a string.
...
#' @param cache A caching function used to store larger intermediate data (by
#'        default these are not stored)
read_gff <- function(file, tags, cache=identity){
...
# Here we build a function to recache the last value stored in an Rmonad
# object. We choose to cache our data locally in the 'ARCHIVE' folder. `rmonad`
# will create the folder and the caching files automatically.
cache <- make_recacher(make_local_cacher('ARCHIVE'))
result <- read_gff(file=good_gfffile, tags=c("ID", "Parent"), cache=cache)
```
...
After we are happy with our result, we may want to remove the cached data:
...
```{r}
m <-
  system.file('extdata', 'gff', '15.gff3', package='rmonad') %>%
  read_gff(tags=c("ID", "Parent"))
```

This fails. To diagnose the problem, we might first access the error message:

```{r}
missues(m)
```

The `id` column above can be used to access attributes of the failing node:
...
```{r}
get_value(m, 2)[[1]] %>% head(5)
get_value(m, 2)[[1]] %>% tail(5)
```
