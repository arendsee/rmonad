...
gff_as_graph <- function(file, cache=identity){
  read_gff(file, tags=c("ID", "Parent"), cache=cache) %>% cache %>>%
...
result <-
  system.file('extdata', 'gff', '0.gff3', package='rmonad') %v>%
  gff_as_graph(cache=cache)
result %>>% plot
```
