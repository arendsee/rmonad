...
read_gff <- function(file, tags){
  ...
  # globals - rmonad will eventually have a better way of handling this
  # rmonad's branching and joining handling is still a little awkward
  # also I don't fully understand what I have created yet
  globals <- list()

  {

    "Rmonad supports docstrings. If an block begins with a string, this string
    is extracted and stored. Python has something similar, where the first
    string in a function is cast as documentation."
    ...
    merge(data_frame(order=1:nrow(globals$gff)), all=TRUE) %>%
    dplyr::arrange(order) %>%
    { cbind(g, .) } %>%
    dplyr::select(-.U, -order, -ntags, -attr)
