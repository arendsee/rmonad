...
read_gff <- function(file, tags, cache=identity){
  ...
  raw_gff %>% cache %>>% {
    ...
    "
    According the the GFF specification, the value in the type column should be
    from the Sequence Ontology (SO). Here I collapse the the synonymous terms
    ...
    changed, which is safer and also clarifies out intention. 
    "
...
result <- read_gff(file=gff$good, tags=c("ID", "Parent"), cache=cache)
```
