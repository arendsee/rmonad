...
to understand. If you are not familiar with them, you may try reading a few of
the many online tutorials. That said, `rmonad` can be used without
 ...
 * when x is numeric and $x \ge 0$ : $\sqrt{x}$ 
 * when x is numeric and $x < 0$ : `NaN` with a warning
 * when x is not numeric : an error that stops evaluation and dumps a message

Every R function maps a pure input value to a *computational context* with
possible undefined behavior and side effects. We can describe the action of a
function abstractly as
...
either a number or some effect.
...
We can represent the function in bastardized Haskell syntax as follows:
...
dynamically typed, we have no guarantees that the input actually is numeric.
...
The initial %>>% operators acts as both a `return` and `bind` function. It
first evaluates the input x.
